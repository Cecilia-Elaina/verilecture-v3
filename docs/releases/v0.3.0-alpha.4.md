...
The public README and website now include accepted Windows x64 result captures showing review points, transcript context, source timestamps, and the return-to-audio interaction. The generated result-screen concept mockup remains marked `概念示意 · 非实机截图` as a design reference only. This evidence is limited to the documented Windows x64 path and does not extend to Linux/macOS local-ASR runtime acceptance; see the [screenshot brief](../../docs/brand/SCREENSHOT_BRIEF.md).
