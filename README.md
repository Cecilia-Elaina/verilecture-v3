...
桌面应用已接入 Windows、Linux 和 macOS 的原生构建矩阵。当前公开的本地转写运行时仍是 Windows x64 版本，因此 Linux 和 macOS 安装包暂时提供桌面应用，原生本地转写运行时待单独验证后发布。

| 平台 | 安装包 | 当前本地转写状态 |
| --- | --- | --- |
| Windows x64 | NSIS | Fun-ASR 与 CUDA Runtime 路径可用 |
| Linux x64 | AppImage | 原生桌面包可构建；本地转写运行时待发布 |
| macOS | DMG | 原生桌面包可构建；本地转写运行时待发布 |
...
`v0.3.0-alpha.1` 是 V3 的第一个公开里程碑，不是最终稳定版。三平台原生构建已纳入 CI；Windows x64 的本地工作流、引导、硬件路由、CPU 路径和代表性 CUDA 路径已完成。Linux/macOS 的本地转写运行时、CUDA Runtime 公共托管和干净环境安装仍需单独验收。
